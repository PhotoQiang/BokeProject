...
import com.util.IsNumber;

public class GoPageFriendServlet extends HttpServlet {
	
	FriendDao fd=new FriendDao();
	IsNumber isnum=new IsNumber();
	public GoPageFriendServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		User user=(User)request.getSession().getAttribute("user");
	       ...
	       if(goPage!=null&&isnum.ifNumber(goPage)){
	    	   System.out.println("得到去哪一页");
	    	  int gogo=Integer.parseInt(goPage);
	    	  if(gogo<=cpage.getTotalPage()&&gogo>=1){
	    		  cpage.setCurrentPage(gogo);
	    	  }else{
